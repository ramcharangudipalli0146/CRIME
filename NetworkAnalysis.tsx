...
import { getNeighbors, getConnections, shortestPath, getAttentionBreakdown, getCallContext } from '@/analytics/networkAnalytics';
  ...
  */
  ...
  useEffect(() => {
    ...
    cy.on('mouseover', 'edge', (evt: EventObject) => {
      ...
      const bbox = (containerRef.current as HTMLElement).getBoundingClientRect();
      const position = edge.midpoint();
      const pan = cy.pan();
      const zoom = cy.zoom();
      const x = bbox.left + position.x * zoom + pan.x;
      const y = bbox.top + position.y * zoom + pan.y;
      setEdgeTooltip({
        visible: true,
        x,
        y,
        ...
        relationship: edge.data('type') as string,
        weight: edge.data('weight') as number,
        strength: edge.data('strength') as string,
      });
    ...
    cy.on('mouseout', 'edge', (evt: EventObject) => {
      ...
      edge.connectedNodes().removeClass('highlighted');
      setEdgeTooltip(null);
    });
  ...
  return (
    <div className="flex h-full">
      ...
      <div className="flex-1 relative min-w-0">
        ...
        {/* Edge hover tooltip */}
        {edgeTooltip?.visible && dataset && (() => {
          const srcEntity = dataset.entities.find(e => e.id === edgeTooltip.source);
          const tgtEntity = dataset.entities.find(e => e.id === edgeTooltip.target);
          const isPhoneEdge = srcEntity?.type === 'phone' || tgtEntity?.type === 'phone';
          const callRecords = edgeTooltip.relationship === 'called'
            ? getCallContext(edgeTooltip.source, edgeTooltip.target, dataset)
            : [];
          return (
            <div
            ...
            >
              <div className="glass-panel p-3 rounded-lg shadow-xl max-w-xs border border-ink-600">
                <div className="text-xs font-semibold text-gray-200 mb-1.5">
                  {edgeTooltip.relationship.replace(/_/g, ' ').toUpperCase()}
                </div>
                <div className="text-[10px] text-gray-400 space-y-0.5">
                  <div><span className="text-gray-500">From:</span> <span className="text-accent-400 font-mono">{edgeTooltip.source}</span> {srcEntity?.label ? `(${srcEntity.label})` : ''}</div>
                  <div><span className="text-gray-500">To:</span> <span className="text-accent-400 font-mono">{edgeTooltip.target}</span> {tgtEntity?.label ? `(${tgtEntity.label})` : ''}</div>
                  <div><span className="text-gray-500">Strength:</span> <span className={`font-mono ${
                    edgeTooltip.strength === 'high' ? 'text-signal-red' : edgeTooltip.strength === 'medium' ? 'text-signal-amber' : 'text-signal-green'
                  }`}>{edgeTooltip.strength}</span></div>
                  <div><span className="text-gray-500">Weight:</span> <span className="text-gray-300 font-mono">{edgeTooltip.weight}</span></div>
                </div>

                {/* Call context for edges involving phone nodes or called relationships */}
                {(isPhoneEdge || edgeTooltip.relationship === 'called') && callRecords.length > 0 && (
                  <div className="mt-2 pt-2 border-t border-ink-600">
                    <div className="text-[10px] font-semibold text-signal-green mb-1">
                      <Phone className="w-3 h-3 inline mr-1" />Call Context ({callRecords.length} records)
                    </div>
                    <div className="max-h-36 overflow-y-auto space-y-1">
                      {callRecords.slice(0, 8).map((cdr) => (
                        <div key={cdr.id} className="p-1.5 rounded bg-ink-850 border border-ink-700 text-[10px]">
                          <div className="flex items-center justify-between">
                            <span className="text-gray-300 font-mono">{cdr.id}</span>
                            <span className={`capitalize ${
                              cdr.callType === 'missed' ? 'text-signal-red' : cdr.callType === 'incoming' ? 'text-signal-green' : 'text-accent-400'
                            }`}>{cdr.callType}</span>
                          </div>
                          <div className="text-gray-500 mt-0.5">
                            {new Date(cdr.timestamp).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: '2-digit' })}{' '}{new Date(cdr.timestamp).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}
                            {' · '}{Math.floor(cdr.duration / 60)}m {cdr.duration % 60}s
                          </div>
                          <div className="text-gray-600">📍 {cdr.location}</div>
                        </div>
                      ))}
                      {callRecords.length > 8 && (
                        <div className="text-[9px] text-gray-600 text-center">+{callRecords.length - 8} more records</div>
                      )}
                    </div>
                  </div>
                )}
